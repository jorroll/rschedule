import { ArgumentError, cloneRuleOptions, DateTime } from '@rschedule/core';
import { IRuleArgs, RuleBase } from '@rschedule/core/generators';
import { ICAL_RULES, INormRRuleOptions, IRRuleOptions } from '@rschedule/core/rules/ICAL_RULES';

export { INormRRuleOptions, IRRuleOptions };

export class RRule<Data = any> extends RuleBase<IRRuleOptions, INormRRuleOptions, Data> {
   ...
   *
   */
  constructor(config: IRRuleOptions, options: IRuleArgs<Data> & { duration?: number } = {}) {
    super(ICAL_RULES, config, options);

    this.duration = options.duration || 0;
    this.hasDuration = !!this.duration;

    if ((config.frequency as any) === 'MILLISECONDLY') {
      throw new ArgumentError(`RRule cannot have a frequency of "MILLISECONDLY".`);
    }
  }
   ...
   * `start` time.
   */
  set(prop: 'timezone', value: string | null, tzoptions?: { keepLocalTime?: boolean }): RRule<Data>;
  set(prop: 'options', value: IRRuleOptions): RRule<Data>;
  set(prop: 'duration', value: number): RRule<Data>;
  set<Prop extends keyof IRRuleOptions>(prop: Prop, value: IRRuleOptions[Prop]): RRule<Data>;
  set<Prop extends keyof IRRuleOptions | 'timezone' | 'options'>(
    prop: Prop,
    ...
    if (prop === 'timezone') {
      ...
      options = (value as any) as IRRuleOptions;
    } else if (prop === 'duration') {
      duration = value as number;
    } else {
      options[prop as Exclude<Prop, 'timezone' | 'options'>] = value as IRRuleOptions[Exclude<
        Prop,
        'timezone' | 'options'
      >];
    }
