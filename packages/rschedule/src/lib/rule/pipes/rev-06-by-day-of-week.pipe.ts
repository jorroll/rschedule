...
export class ByDayOfWeekReversePipe extends ReversePipeRule
  ...
  public weeklyExpand(args: IPipeRunFn) {
    ...
    if (this.preceedingDays.length === 0) {
      const orderedWeekdays = Utils.orderedWeekdays(
      ...
      this.preceedingDays = this.options
        .byDayOfWeek!.map(day =>
          orderedWeekdays.indexOf(day as DateTime.Weekday),
        )
    ...
    Utils.setDateToEndOfWeek(date, this.options.weekStart).subtract(
      nextDay,
      'day',
    );
  ...
  public monthlyFilter(args: IPipeRunFn) {
    ...
    const next = previousValidDateThisMonth
      ...
      : getPreviousValidDateThisMonth(
          this.cloneDateWithGranularity(args.date, 'month')
            .set('day', 1)
            .subtract(1, 'day'),
          this.options,
          this.cachedValidMonthDays,
  ...
  public simpleFilter(args: IPipeRunFn) {
    ...
    const validWeekdays = weekdays.filter(day =>
      (this.options.byDayOfWeek as DateTime.Weekday[]).includes(day),
    );
...
function getValidYearDays(date: DateTime, options: PipeControllerOptions) {
...
function getValidMonthDays(date: DateTime, options: PipeControllerOptions) {
  const weekdays: DateTime.Weekday[] = [];
  const specificWeekdays: Array<[DateTime.Weekday, number]> = [];
  let hasPositiveWeekdays = false;
  let hasNegativeWeekdays = false;
