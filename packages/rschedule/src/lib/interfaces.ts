...
export interface IHasOccurrences<T extends DateAdapterConstructor>
...
export abstract class HasOccurrences<T extends DateAdapterConstructor> {
  public abstract isInfinite: boolean;
  ...
  public occurrences(args: OccurrencesArgs<T> = {}): OccurrenceIterator<T> {
    return new OccurrenceIterator(
      this as any,
      this.processOccurrencesArgs(args),
    );
  }
  ...
  public occursBetween(
    ...
    for (const day of this._run({ start: startAdapter, end: endAdapter })) {
      if (options.excludeEnds) {
        if (day.isEqual(startAdapter)) {
          continue;
        }
        if (day.isEqual(endAdapter)) {
          break;
        }
      }
  ...
  public abstract _run(args?: RunArgs<T>): IterableIterator<DateAdapter<T>>;

  /**
   * Checks to see if an occurrence exists which equals the given date.
   */
  public abstract occursOn(args: { date: DateProp<T> | DateAdapter<T> }): boolean;
  /**
  ...
  protected processOccursOnArgs(
    ...
    excludeEnds?: boolean;
    excludeDates?: Array<DateAdapter<T>>;
  } {
    return {
      ...rawArgs,
      ...
      excludeDates:
        ...
        rawArgs.excludeDates.map(
          date => this.buildDateAdapter(date) as DateAdapter<T>,
        ),
    };
  }
...
export class OccurrenceIterator<T extends DateAdapterConstructor> {
  ...
  public toArray() {
    if (!this.args.end && !this.args.take && this.iterable.isInfinite) {
      ...
      const occurrences: Array<DateAdapter<T>> = [];
