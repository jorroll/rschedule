...
import { OperatorOutput, OperatorObject, OccurrenceStream } from "./interface";
...
export function occurrenceStream<T extends DateAdapterConstructor>(
  ...
  return (dateAdapter: T) => {
    ...
    switch (operators.length) {
      ...
      default:
        first = operators.shift()!({dateAdapter})
        _run = (args: RunArgs<T>={}) => operators.reduce((prev, curr) => curr({dateAdapter, base: prev})._run(args), first._run(args))
        break
    }
    ...
    return {
      ...
      get isInfinite() {
        ...
        try {
          operators.reduce((prev, curr) => {
            const isInfinite = curr({dateAdapter, baseIsInfinite: prev}).isInfinite
      ...
      setTimezone(timezone: string | undefined, options?: {keepLocalTime?: boolean}) {
        operators.forEach(operator => operator({dateAdapter}).setTimezone(timezone, options))
      },
    }
  }
}
