 ...
 */
export class Dates<T extends DateAdapterConstructor, D = any>
  ...
  constructor(
    args: {
      dates?: Array<DateProp<T> | DateAdapter<T>>;
      data?: D;
      dateAdapter?: T;
    } = {},
  ) {
    ...
    if (args.dates) {
      this.dates = args.dates.map(date =>
        DateAdapterBase.isInstance(date) ? date.date : date,
      );
    }
    ...
    if (args.weekday) {
      ...
      return this.dates.some(date => {
        ...
        return (
          ...
          (!args.excludeDates ||
            !args.excludeDates.some(exdate => exdate.isEqual(adapter))) &&
  ...
  public *_run(args: RunArgs<T> = {}) {
    let dates = Utils.sortDates(
      uniqWith(this.dates.map(date => new this.dateAdapter(date)), (a, b) =>
        a.isEqual(b),
      ),
    );
