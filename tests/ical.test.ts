...
import { datetime, test } from './utilities';
...
describe('parseDatetime()', () => {
  ...
  describe('INVALID', () => {
    test('20000307132405', time => {
      expect(() => parseDatetime(time)).toThrowError(
        `Invalid ICAL date/time string "${time}"`,
      );
    });
...
describe('parseFrequency()', () => {
  ...
  describe('INVALID', () => {
    test('SECONDL', text => {
      expect(() => parseFrequency(text)).toThrowError(
        `Invalid FREQ value "${text}"`,
      );
    });
  });
});
...
describe('parseUntil()', () => {
  describe('VALID', () => {
    test('19970902T090000Z', text => {
      expect(
        parseUntil(
          text,
          StandardDateAdapter,
          new StandardDateAdapter(new Date()),
        ).toISOString(),
      ).toBe(
        new StandardDateAdapter(
          new Date(Date.UTC(1997, 8, 2, 9)),
        ).toISOString(),
      );
    });
    ...
    test('19970902T090000', text => {
      ...
      ).toBe(new StandardDateAdapter(new Date(1997, 8, 2, 9)).toISOString());
    });
  });
  ...
  describe('INVALID', () => {
    test('19970902090000', text => {
      ...
      ).toThrowError(`Invalid ICAL date/time string "${text}"`);
    });
  });
});
...
describe('parseCount()', () => {
  ...
  describe('INVALID', () => {
    test('SECONDL', text => {
      expect(() => parseCount(text)).toThrowError(
...
describe('parseInterval()', () => {
  ...
  describe('INVALID', () => {
    test('SECONDL', text => {
      expect(() => parseInterval(text)).toThrowError(
...
describe('parseBySecond()', () => {
  ...
  describe('INVALID', () => {
    test('SECONDL', text => {
      expect(() => parseBySecond(text)).toThrowError(
...
describe('parseByMinute()', () => {
  ...
  describe('INVALID', () => {
    test('SECONDL', text => {
      expect(() => parseByMinute(text)).toThrowError(
...
describe('parseByMonth()', () => {
  ...
  describe('INVALID', () => {
    test('SECONDL', text => {
      expect(() => parseByMonth(text)).toThrowError(
...
describe('parseWkst()', () => {
  ...
  describe('INVALID', () => {
    test('SECONDL', text => {
      expect(() => parseWkst(text)).toThrowError(
