import {
  parseICalStrings,
  serializeRScheduleToICal,
  serializeRScheduleToJCal,
...
describe('parseICalStrings()', () => {
  it('parses RRULE_STRING', () => {
    expect(
      parseICalStrings(RRULE_STRING, MomentTZDateAdapter, {
  ...
  it('parses RRULE_STRING_TWO', () => {
    expect(
      JSON.parse(
        JSON.stringify(
          parseICalStrings(RRULE_STRING_TWO, MomentTZDateAdapter, {
            returnOptionsObjects: true,
          }),
        ),
      ),
  ...
  it('parses VEVENT_STRING', () => {
    expect(
      parseICalStrings(VEVENT_STRING, MomentTZDateAdapter, {
        returnOptionsObjects: true,
      }),
    ).toEqual([
      {
  ...
  it('parses VCALENDAR_STRING', () => {
    expect(
      parseICalStrings(VCALENDAR_STRING, MomentTZDateAdapter, {
        returnOptionsObjects: true,
      }),
    ).toEqual([
      {
            ...
            schedules: [
              ...
              },
              {
                rrules: [
                  {
                    frequency: 'WEEKLY',
                    start: moment.tz([2018, 9, 10, 10, 0, 0], 'Europe/London'),
                    byDayOfWeek: ['WE'],
                  },
                ],
                exrules: [],
  ...
  it('handles INVALID_STRING_ONE', () => {
    expect(() =>
      parseICalStrings(INVALID_STRING_ONE, MomentTZDateAdapter, {
        returnOptionsObjects: true,
      }),
  ...
  it('handles INVALID_STRING_TWO', () => {
    expect(() =>
      parseICalStrings(INVALID_STRING_TWO, MomentTZDateAdapter, {
        returnOptionsObjects: true,
      }),
  ...
  it('handles INVALID_STRING_THREE', () => {
    expect(
      parseICalStrings(INVALID_STRING_THREE, MomentTZDateAdapter, {
        returnOptionsObjects: true,
      }),
  ...
  it('handles INVALID_STRING_FOUR', () => {
    expect(
      parseICalStrings(INVALID_STRING_FOUR, MomentTZDateAdapter, {
        returnOptionsObjects: true,
      }),
    ).toEqual([{ iCalendar: [], calendars: [], events: [] }]);
  });
  ...
  it('handles INVALID_STRING_FIVE', () => {
    expect(() =>
      parseICalStrings(INVALID_STRING_FIVE, MomentTZDateAdapter, {
        returnOptionsObjects: true,
      }),
    ).toThrowError(
  ...
  it('handles INVALID_STRING_SIX', () => {
    expect(() =>
      parseICalStrings(INVALID_STRING_SIX, MomentTZDateAdapter, {
        returnOptionsObjects: true,
      }),
...
describe('serializes', () => {
  ...
  describe('serializeRScheduleToJCal()', () => {
    it('serializes Schedule', () => {
      expect(serializeRScheduleToJCal(schedule)).toEqual([
        [
          'vevent',
          [
  ...
  describe('serializeRScheduleToICal()', () => {
    it('serializes Schedule', () => {
      const ical = [
        'BEGIN:VEVENT',
      ...
      expect(serializeRScheduleToICal(schedule)).toEqual([ical]);
    });
  });
});
...
describe('deserializes > serializes > deserializes', () => {
  ...
  it('works', () => {
    const parsed = parseICalStrings(ical, StandardDateAdapter)[0]
      .events[0] as Schedule<typeof StandardDateAdapter>;

    const serialized = serializeRScheduleToICal(parsed)[0];
